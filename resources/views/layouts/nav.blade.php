@auth
		<nav class="flex items-center justify-between rounded-lg bg-slate-800 p-4 font-medium text-gray-300">
				<div class="space-x-3">
						<a href="{{ route('home.create') }}" class="hover:text-white">home</a>
						<a href="{{ route('setting.create', auth()->user()->slug) }}" class="hover:text-white">settings
						</a>
				</div>
				<div x-data="dropdown" x-on:click.away="away()" class="relative">
						...
						<div x-show="open" x-transition.duration.500ms
								...
								<a href="{{ route('profile.show', auth()->user()->slug) }}" class="m-2 block text-left hover:text-white">
